...
public class MenuCoreService {
    ...
    public Menu createMenu(MenuRequest menuRequest) throws IOException {
        ...
        log.info("{}\nuploadImgUrl={}\nmenuSavedurl={}", saved.getMenuName(), uploadImgUrl,
                saved.getMenuImg().getImgUrl());
        return saved;
    ...
    public void updateStock(String menuName, Integer newStock) {
        Menu menu = findMenuByMenuName(menuName);
        if (menu == null) {
            throw new GlobalException(ErrorCode.NOT_FOUND_MENU);
        }
        menu.updateStock(newStock);
    }
}
